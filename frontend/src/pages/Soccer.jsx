...
const Soccer = () => {
    ...
    const [sortType, setSortType] = useState(() => {
        const saved = sessionStorage.getItem("soccerSortType");
        return saved || "relevant";
    });

    const [currentPage, setCurrentPage] = useState(() => {
        const saved = sessionStorage.getItem("soccerCurrentPage");
        return saved ? parseInt(saved, 10) : 1;
    });

    const [category, setCategory] = useState(() => {
        const saved = sessionStorage.getItem("soccerCategory");
        return saved ? JSON.parse(saved) : [];
    });

    const [subCategory, setSubCategory] = useState(() => {
        const saved = sessionStorage.getItem("soccerSubCategory");
        return saved ? JSON.parse(saved) : [];
    });
    ...
    useEffect(() => {
        sessionStorage.setItem("soccerCategory", JSON.stringify(category));
    }, [category]);

    useEffect(() => {
        sessionStorage.setItem("soccerSubCategory", JSON.stringify(subCategory));
    }, [subCategory]);

    useEffect(() => {
        sessionStorage.setItem("soccerCurrentPage", currentPage);
    }, [currentPage]);

    useEffect(() => {
        sessionStorage.setItem("soccerSortType", sortType);
    }, [sortType]);

    // --- Clear sessionStorage on unmount ---
    useEffect(() => {
        return () => {
            sessionStorage.removeItem("soccerCategory");
            sessionStorage.removeItem("soccerSubCategory");
            sessionStorage.removeItem("soccerCurrentPage");
            sessionStorage.removeItem("soccerSortType");
        };
    }, []);
    ...
    useEffect(() => {
        ...
        productsCopy = productsCopy.filter((item) => item.sportsCategory?.toLowerCase().trim() === "soccer");
    ...
    return (
        <>
            <div className="relative w-full h-[150px] sm:h-[200px] md:h-[250px] lg:h-[285px]">
                <img src={assets.soccer} alt="Soccer" className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/40 to-transparent"></div>
                <div className="absolute inset-0 flex items-center justify-start px-6 sm:px-12">
                    <h1 className="text-white text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-wide">SOCCER</h1>
                </div>
            </div>
