...
import Banner from "../components/Soccer/Banner.jsx";
...
const Soccer = () => {
    ...
    const [sortType, setSortType] = useState(() => sessionStorage.getItem("soccerSortType") || "relevant");
    const [currentPage, setCurrentPage] = useState(
        () => parseInt(sessionStorage.getItem("soccerCurrentPage"), 10) || 1
    );
    const [category, setCategory] = useState(() => JSON.parse(sessionStorage.getItem("soccerCategory")) || []);
    const [subCategory, setSubCategory] = useState(() => JSON.parse(sessionStorage.getItem("soccerSubCategory")) || []);

    const productsRef = useRef(null);

    // Persist states
    useEffect(() => sessionStorage.setItem("soccerCategory", JSON.stringify(category)), [category]);
    useEffect(() => sessionStorage.setItem("soccerSubCategory", JSON.stringify(subCategory)), [subCategory]);
    useEffect(() => sessionStorage.setItem("soccerCurrentPage", currentPage), [currentPage]);
    useEffect(() => sessionStorage.setItem("soccerSortType", sortType), [sortType]);

    // Cleanup
    useEffect(() => {
        return () => {
            ["soccerCategory", "soccerSubCategory", "soccerCurrentPage", "soccerSortType"].forEach((key) =>
    ...
    // Filtering
    useEffect(() => {
        let copy = products.filter((p) => p.sportsCategory?.toLowerCase() === "soccer");
