...
import Banner from "../components/Activewear/Banner.jsx";
...
const Activewear = () => {
    ...
    const [sortType, setSortType] = useState(() => sessionStorage.getItem("activewearSortType") || "relevant");
    const [currentPage, setCurrentPage] = useState(
        () => parseInt(sessionStorage.getItem("activewearCurrentPage"), 10) || 1
    );
    const [category, setCategory] = useState(() => JSON.parse(sessionStorage.getItem("activewearCategory")) || []);
    const [subCategory, setSubCategory] = useState(
        () => JSON.parse(sessionStorage.getItem("activewearSubCategory")) || []
    );

    const productsRef = useRef(null);

    // Persist states
    useEffect(() => sessionStorage.setItem("activewearCategory", JSON.stringify(category)), [category]);
    useEffect(() => sessionStorage.setItem("activewearSubCategory", JSON.stringify(subCategory)), [subCategory]);
    useEffect(() => sessionStorage.setItem("activewearCurrentPage", currentPage), [currentPage]);
    useEffect(() => sessionStorage.setItem("activewearSortType", sortType), [sortType]);

    // Cleanup
    useEffect(() => {
        return () => {
            ["activewearCategory", "activewearSubCategory", "activewearCurrentPage", "activewearSortType"].forEach(
    ...
    // Filtering
    useEffect(() => {
        let copy = products.filter((p) => p.sportsCategory?.toLowerCase() === "activewear");
    ...
    return (
        <>
            <Banner baseImg={assets.activewear} />
