...
const Basketball = () => {
    ...
    const [sortType, setSortType] = useState(() => {
        const saved = sessionStorage.getItem("basketballSortType");
        return saved || "relevant";
    });

    const [currentPage, setCurrentPage] = useState(() => {
        const saved = sessionStorage.getItem("basketballCurrentPage");
        return saved ? parseInt(saved, 10) : 1;
    });

    const [category, setCategory] = useState(() => {
        const saved = sessionStorage.getItem("basketballCategory");
        return saved ? JSON.parse(saved) : [];
    });

    const [subCategory, setSubCategory] = useState(() => {
        const saved = sessionStorage.getItem("basketballSubCategory");
        return saved ? JSON.parse(saved) : [];
    });
    ...
    useEffect(() => {
        sessionStorage.setItem("basketballCategory", JSON.stringify(category));
    }, [category]);

    useEffect(() => {
        sessionStorage.setItem("basketballSubCategory", JSON.stringify(subCategory));
    }, [subCategory]);

    useEffect(() => {
        sessionStorage.setItem("basketballCurrentPage", currentPage);
    }, [currentPage]);

    useEffect(() => {
        sessionStorage.setItem("basketballSortType", sortType);
    }, [sortType]);

    // --- Clear sessionStorage on unmount ---
    useEffect(() => {
        return () => {
            sessionStorage.removeItem("basketballCategory");
            sessionStorage.removeItem("basketballSubCategory");
            sessionStorage.removeItem("basketballCurrentPage");
            sessionStorage.removeItem("basketballSortType");
        };
    }, []);
    ...
    useEffect(() => {
        ...
        productsCopy = productsCopy.filter((item) => item.sportsCategory?.toLowerCase().trim() === "basketball");
    ...
    return (
        <>
            <div className="relative w-full h-[150px] sm:h-[200px] md:h-[250px] lg:h-[285px] border-t border-white">
                <img src={assets.men_basketball} alt="Basketball" className="w-full h-full object-cover" />
