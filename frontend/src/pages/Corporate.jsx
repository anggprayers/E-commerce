...
import Banner from "../components/Corporate/Banner.jsx";
...
const Corporate = () => {
    ...
    const [sortType, setSortType] = useState(() => sessionStorage.getItem("corporateSortType") || "relevant");
    const [currentPage, setCurrentPage] = useState(
        () => parseInt(sessionStorage.getItem("corporateCurrentPage"), 10) || 1
    );
    const [category, setCategory] = useState(() => JSON.parse(sessionStorage.getItem("corporateCategory")) || []);
    const [subCategory, setSubCategory] = useState(
        () => JSON.parse(sessionStorage.getItem("corporateSubCategory")) || []
    );

    const productsRef = useRef(null);

    // Persist states
    useEffect(() => sessionStorage.setItem("corporateCategory", JSON.stringify(category)), [category]);
    useEffect(() => sessionStorage.setItem("corporateSubCategory", JSON.stringify(subCategory)), [subCategory]);
    useEffect(() => sessionStorage.setItem("corporateCurrentPage", currentPage), [currentPage]);
    useEffect(() => sessionStorage.setItem("corporateSortType", sortType), [sortType]);

    // Cleanup
    useEffect(() => {
        return () => {
            ["corporateCategory", "corporateSubCategory", "corporateCurrentPage", "corporateSortType"].forEach((key) =>
    ...
    // Filtering
    useEffect(() => {
        let copy = products.filter((p) => p.sportsCategory?.toLowerCase() === "corporate");
    ...
    return (
        <>
            <Banner baseImg={assets.corporate} />
