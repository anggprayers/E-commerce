...
const Corporate = () => {
    ...
    const [sortType, setSortType] = useState(() => {
        const saved = sessionStorage.getItem("corporateSortType");
        return saved || "relevant";
    });

    const [currentPage, setCurrentPage] = useState(() => {
        const saved = sessionStorage.getItem("corporateCurrentPage");
        return saved ? parseInt(saved, 10) : 1;
    });

    const [category, setCategory] = useState(() => {
        const saved = sessionStorage.getItem("corporateCategory");
        return saved ? JSON.parse(saved) : [];
    });

    const [subCategory, setSubCategory] = useState(() => {
        const saved = sessionStorage.getItem("corporateSubCategory");
        return saved ? JSON.parse(saved) : [];
    });
    ...
    useEffect(() => {
        sessionStorage.setItem("corporateCategory", JSON.stringify(category));
    }, [category]);

    useEffect(() => {
        sessionStorage.setItem("corporateSubCategory", JSON.stringify(subCategory));
    }, [subCategory]);

    useEffect(() => {
        sessionStorage.setItem("corporateCurrentPage", currentPage);
    }, [currentPage]);

    useEffect(() => {
        sessionStorage.setItem("corporateSortType", sortType);
    }, [sortType]);

    // --- Clear sessionStorage on unmount ---
    useEffect(() => {
        return () => {
            sessionStorage.removeItem("corporateCategory");
            sessionStorage.removeItem("corporateSubCategory");
            sessionStorage.removeItem("corporateCurrentPage");
            sessionStorage.removeItem("corporateSortType");
        };
    }, []);
    ...
    useEffect(() => {
        ...
        productsCopy = productsCopy.filter((item) => item.sportsCategory?.toLowerCase().trim() === "corporate");
    ...
    return (
        <>
            <div className="relative w-full h-[150px] sm:h-[200px] md:h-[250px] lg:h-[285px]">
                <img src={assets.corporate} alt="Corporate" className="w-full h-full object-cover" />
