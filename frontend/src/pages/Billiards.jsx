...
import Banner from "../components/Billiards/Banner.jsx";
...
const Billiards = () => {
    ...
    const [sortType, setSortType] = useState(() => sessionStorage.getItem("billiardsSortType") || "relevant");
    const [currentPage, setCurrentPage] = useState(
        () => parseInt(sessionStorage.getItem("billiardsCurrentPage"), 10) || 1
    );
    const [category, setCategory] = useState(() => JSON.parse(sessionStorage.getItem("billiardsCategory")) || []);
    const [subCategory, setSubCategory] = useState(
        () => JSON.parse(sessionStorage.getItem("billiardsSubCategory")) || []
    );

    const productsRef = useRef(null);

    // Persist states
    useEffect(() => sessionStorage.setItem("billiardsCategory", JSON.stringify(category)), [category]);
    useEffect(() => sessionStorage.setItem("billiardsSubCategory", JSON.stringify(subCategory)), [subCategory]);
    useEffect(() => sessionStorage.setItem("billiardsCurrentPage", currentPage), [currentPage]);
    useEffect(() => sessionStorage.setItem("billiardsSortType", sortType), [sortType]);

    // Cleanup
    useEffect(() => {
        return () => {
            ["billiardsCategory", "billiardsSubCategory", "billiardsCurrentPage", "billiardsSortType"].forEach((key) =>
    ...
    // Filtering
    useEffect(() => {
        let copy = products.filter((p) => p.sportsCategory?.toLowerCase() === "billiards");
    ...
    return (
        <>
            <Banner baseImg={assets.billiards} />
