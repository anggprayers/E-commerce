...
const Billiards = () => {
    ...
    const baseImg = assets.billiards;

    // --- Persisted States ---
    const [sortType, setSortType] = useState(() => {
        const saved = sessionStorage.getItem("billiardsSortType");
        return saved || "relevant";
    });

    const [currentPage, setCurrentPage] = useState(() => {
        const saved = sessionStorage.getItem("billiardsCurrentPage");
        return saved ? parseInt(saved, 10) : 1;
    });

    const [category, setCategory] = useState(() => {
        const saved = sessionStorage.getItem("billiardsCategory");
        return saved ? JSON.parse(saved) : [];
    });

    const [subCategory, setSubCategory] = useState(() => {
        const saved = sessionStorage.getItem("billiardsSubCategory");
        return saved ? JSON.parse(saved) : [];
    });
    ...
    useEffect(() => {
        sessionStorage.setItem("billiardsCategory", JSON.stringify(category));
    }, [category]);

    useEffect(() => {
        sessionStorage.setItem("billiardsSubCategory", JSON.stringify(subCategory));
    }, [subCategory]);

    useEffect(() => {
        sessionStorage.setItem("billiardsCurrentPage", currentPage);
    }, [currentPage]);

    useEffect(() => {
        sessionStorage.setItem("billiardsSortType", sortType);
    }, [sortType]);

    // --- Clear sessionStorage on unmount ---
    useEffect(() => {
        return () => {
            sessionStorage.removeItem("billiardsCategory");
            sessionStorage.removeItem("billiardsSubCategory");
            sessionStorage.removeItem("billiardsCurrentPage");
            sessionStorage.removeItem("billiardsSortType");
        };
    }, []);
    ...
    useEffect(() => {
        ...
        productsCopy = productsCopy.filter((item) => item.sportsCategory?.toLowerCase().trim() === "billiards");
