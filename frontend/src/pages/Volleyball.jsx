...
import Banner from "../components/Volleyball/Banner.jsx";
...
const Volleyball = () => {
    ...
    const [sortType, setSortType] = useState(() => sessionStorage.getItem("volleyballSortType") || "relevant");
    const [currentPage, setCurrentPage] = useState(
        () => parseInt(sessionStorage.getItem("volleyballCurrentPage"), 10) || 1
    );
    const [category, setCategory] = useState(() => JSON.parse(sessionStorage.getItem("volleyballCategory")) || []);
    const [subCategory, setSubCategory] = useState(
        () => JSON.parse(sessionStorage.getItem("volleyballSubCategory")) || []
    );

    const productsRef = useRef(null);

    // Persist states
    useEffect(() => sessionStorage.setItem("volleyballCategory", JSON.stringify(category)), [category]);
    useEffect(() => sessionStorage.setItem("volleyballSubCategory", JSON.stringify(subCategory)), [subCategory]);
    useEffect(() => sessionStorage.setItem("volleyballCurrentPage", currentPage), [currentPage]);
    useEffect(() => sessionStorage.setItem("volleyballSortType", sortType), [sortType]);

    // Cleanup
    useEffect(() => {
        return () => {
            ["volleyballCategory", "volleyballSubCategory", "volleyballCurrentPage", "volleyballSortType"].forEach(
    ...
    // Filtering
    useEffect(() => {
        let copy = products.filter((p) => p.sportsCategory?.toLowerCase() === "volleyball");
    ...
    return (
        <>
            <Banner baseImg={assets.volleyball} />
